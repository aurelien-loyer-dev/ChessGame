 ...
 * Niveaux 3-5 : Stockfish serveur /api/ai-move, fallback WASM
 */
...
// PST tables for minimax (levels 1-2)
var PAWN_TABLE = [
...
var PIECE_VALUES_AI = { P: 100, N: 320, B: 330, R: 500, Q: 900, K: 20000 };


// StockfishBridge — WASM Web Worker fallback
var StockfishBridge = (function () {
  ...
  SB.prototype.init = function () {
    ...
    this.readyPromise = new Promise(function (resolve, reject) {
      ...
      setTimeout(function () { if (!self.ready) reject(new Error('Stockfish WASM timeout')); }, 15000);
  ...
  SB.prototype._onMessage = function (msg) {
    if (msg.type === 'ready') { this.ready = true; if (this._readyResolve) { this._readyResolve(); this._readyResolve = null; } }
    if (msg.type === 'bestmove' && this._searchResolve) { this._searchResolve(msg); this._searchResolve = null; }
  };
...
export var ChessAI = (function () {
  ...
  AI.prototype._getWasmOptions = function () {
    switch (this.difficulty) {
      case AI_DIFFICULTY.HARD: return { skillLevel: 10, depth: 12, movetime: 2000 };
      case AI_DIFFICULTY.EXPERT: return { skillLevel: 18, depth: 16, movetime: 5000 };
      case AI_DIFFICULTY.GRANDMASTER: return { skillLevel: 20, depth: 20, movetime: 10000 };
      default: return { skillLevel: 10, depth: 12, movetime: 2000 };
    }
  };
  ...
  AI.prototype._findWithMinimax = function (engine, aiColor, allMoves) {
    ...
    for (var i = 0; i < allMoves.length; i++) {
      ...
      if (score > bestScore) { bestScore = score; bestMoves = [{ from: move.from, to: move.to, promotion: promo }]; }
      else if (score === bestScore) { bestMoves.push({ from: move.from, to: move.to, promotion: promo }); }
      alpha = Math.max(alpha, score);
    }

    if (bestMoves.length === 0) { var m = allMoves[0]; return { from: m.from, to: m.to, promotion: m.promotion ? 'Q' : undefined }; }
  ...
  AI.prototype._minimax = function (engine, depth, alpha, beta, maximizing, aiColor) {
    ...
    if ((this.nodesSearched & 1023) === 0 && Date.now() - this.timeStart > this.timeBudget) {
      this.aborted = true; return this._eval(engine.board, aiColor);
    }
    var moves = engine.getAllLegalMoves();
    if (moves.length === 0) return engine.isInCheck(engine.turn) ? (maximizing ? -100000 + depth : 100000 - depth) : 0;
    if (depth === 0) return this._eval(engine.board, aiColor);

    moves.sort(function (a, b) {
      var sa = 0, sb = 0;
      var ta = engine.board[a.to.row][a.to.col], tb = engine.board[b.to.row][b.to.col];
      if (ta) sa = PIECE_VALUES_AI[ta.type] || 0;
      if (tb) sb = PIECE_VALUES_AI[tb.type] || 0;
      if (a.promotion) sa += 900; if (b.promotion) sb += 900;
      return sb - sa;
    });
    ...
    if (maximizing) {
      ...
      for (i = 0; i < moves.length; i++) {
        ...
        if (val > best) best = val; if (val > alpha) alpha = val; if (beta <= alpha) break;
      }
    } else {
      best = Infinity;
      for (i = 0; i < moves.length; i++) {
        ...
        if (this.aborted) return val;
        if (val < best) best = val; if (val < beta) beta = val; if (beta <= alpha) break;
      }
    }
    return best;
  };
  ...
  AI.prototype._eval = function (board, aiColor) {
    ...
    for (var r = 0; r < 8; r++)
      for (var c = 0; c < 8; c++) {
        ...
        var v = (PIECE_VALUES_AI[p.type] || 0) + bonus;
